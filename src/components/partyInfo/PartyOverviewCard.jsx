...
export default function PartyOverviewCard({ user, party, setParty }) {
  const playerRows = party.map((PC) => (
    <Card key={PC.pcId} className="mb-2">
      ...
      <Card.Footer>
        <DeletePCModal user={user} party={party} setParty={setParty} PC={PC} />
  ...
  return (
    party && user && <Row className="p-2 bg-dark-subtle">{playerRows}</Row>
  );
}
